...
@section('container')
    ...
    <div class="border-y py-3 space-y-5 mb-7">
        ...
        <div class="font-medium text-lg py-6 ps-3 bg-bgRawat">
            {{ \Carbon\Carbon::parse($diagnose->created_at)->isoFormat('dddd, DD MMMM YYYY') }}</div>
        <div class="flex text-sm space-x-4 gap-7">
            ...
            <div class="space-y-4">
                <div>{{ $diagnose->weight }} kg</div>
                <div>{{ $diagnose->breath }} bpm</div>
                <div>{{ $diagnose->pulse }} kali per menit</div>
                <div>{{ $diagnose->temperature }} derajat celcius</div>
            </div>
        </div>
        <div class="space-y-4">
    ...
    @if (!$diagnose->queued && $diagnose->status == 'menunggu')
        <form action="{{ route('inpatient.update', $diagnose->id) }}" method="post">
            @method('PUT')
            @csrf
